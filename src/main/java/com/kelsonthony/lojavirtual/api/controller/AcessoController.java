...
public class AcessoController {
	...
	public ResponseEntity<Acesso> salvarAcesso(@RequestBody Acesso acesso) throws LojaVirtualException {
		
		if(acesso.getId() == null) {
			List<Acesso> acessos = acessoRepository.buscarAcessDesc(acesso.getDescricao().toUpperCase());
			
			if(!acessos.isEmpty()) {
				throw new LojaVirtualException("Já existe Acesso com a descrição: " + acesso.getDescricao());
			}
		}
	...
	public ResponseEntity<Acesso> obterAcesso(@PathVariable("id") Long id) throws LojaVirtualException {
		
		Acesso acesso = acessoRepository.findById(id).orElse(null);
		
		if (acesso == null) {
			throw new LojaVirtualException("Não encontrou Acesso com o código: " + id);
		}
	...
	public ResponseEntity<List<Acesso>> buscarPorDesc(@PathVariable("desc") String desc) {
		...
		List<Acesso> acesso = acessoRepository.buscarAcessDesc(desc.toUpperCase());
